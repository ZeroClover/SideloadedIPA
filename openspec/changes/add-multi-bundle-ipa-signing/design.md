...
The qualification resolved the original entitlement constraint. Unmodified zsign v1.1.1 accepts multiple profiles but only one global `-e`; profile-only signing mapped profiles correctly but failed the exact root/LiveProcess 128-Keychain-Group contract. The selected patch pairs repeated `-e` values with the already per-profile `ZSignAsset` objects and rejects count mismatches before signing. Installable-canary verification also found that upstream writes extension profiles after generating their resource envelopes; revision `.2` moves the existing profile write before `CodeResources` generation and the backend contract now verifies the exact profile SHA-256 seal.
