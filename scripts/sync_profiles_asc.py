...
def fetch_certificates() -> list[dict[str, Any]]:
    ...
    result = run_asc([
        "certificates", "list",
        "--certificate-type", "IOS_DEVELOPMENT,DEVELOPMENT",
        "--paginate"
    ])
...
def create_profile(name: str, bundle_id: str, cert_ids: list[str], device_ids: list[str]) -> str:
    ...
    cmd = [
        ...
        "--profile-type", PROFILE_TYPE,
        "--bundle", bundle_id,
        "--certificate", ",".join(cert_ids),
        "--device", ",".join(device_ids)
    ]
...
def compare_cached_device_lists() -> bool:
    ...
    current_checksum = current.get("checksum") or calculate_device_checksum(current.get("devices", []))
