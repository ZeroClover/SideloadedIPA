...
def _document(manifest: StageManifest) -> dict[str, object]:
    return {
        ...
        "diagnostics": [diagnostic_document(value) for value in manifest.diagnostics],
    }
...
def canonical_stage_manifest_json(manifest: StageManifest) -> bytes:
    if manifest.manifest_sha256 != stage_manifest_sha256(manifest):
        raise DomainError(
            ...
            "stage manifest digest is inconsistent with its contents",
            task_name=manifest.task_name,
            remediation="rebuild the stage manifest from its current evidence",
        )
