...
def _node_result_document(node: SigningNodeResult) -> dict[str, object]:
    return {
        ...
        "diagnostics": [_diagnostic_document(value) for value in node.diagnostics],
    }
...
def _result_document(result: SigningResult) -> dict[str, object]:
    return {
        ...
        "diagnostics": [_diagnostic_document(value) for value in result.diagnostics],
...
def signing_result_sha256(result: SigningResult) -> str:
    ...
    return hashlib.sha256(_canonical_json(_result_document(result))).hexdigest()


def canonical_signing_result_json(result: SigningResult) -> bytes:
    document = _result_document(result)
    document["result_sha256"] = signing_result_sha256(result)
